...
*Internal design review document. February 2026. Revised with implementation feedback and second-pass audit (Feb 27). Updated March 2026: Section C reflects implemented state (Sentry, OTel, RequestContextMiddleware, error boundaries, frontend telemetry).*
...
| Aspect | Status | Detail |
|---|---|---|
| OpenTelemetry Metrics | **Active** | `MeterProvider` in `core/otel.py` with 9 billing instruments (counters, histograms). Lazy-initialized `_BillingMeters` class tracks: `decision_total`, `decision_failed`, `decision_cancelled`, `anomaly_total`, `catalog_miss`, `estimate_delta_pct`, `charge_sparks`, `debit_latency_ms`, `reservation_sparks` |
| Custom metrics endpoint | Admin-only `/api/admin/metrics` | Computes at query time from DB: ops/hour, failure rate, cost/op, anomalies_by_type, avg_delta_pct, catalog_miss_count |
| Infrastructure metrics | Implicit via Azure Monitor | Container Apps → Log Analytics workspace |

**Two metrics systems coexist:**
1. **OTel instruments** (`core/otel.py`): Write-time counters and histograms emitted during billing operations. These are the proper metrics path — they emit at the moment of state change and can be exported to any OTel-compatible backend.
2. **Admin `/api/admin/metrics` endpoint**: Read-time aggregation from DB. Still useful for admin UX but should not be treated as a monitoring system (full table scans, not real-time).

### Tracing

| Aspect | Status | Detail |
|---|---|---|
| OpenTelemetry | **Active** | `TracerProvider` + `MeterProvider` in `core/otel.py`. Auto-instruments: SQLAlchemy, Redis, HTTPX, Requests, FastAPI |
| Sentry Tracing | **Active** | `sentry-sdk[fastapi,celery]` with `CeleryIntegration(propagate_traces=True)`. Sentry owns span lifecycle; OTel spans silently dropped when `instrumenter="sentry"` (default) |
| Correlation ID | **Full** | `trace_id` generated by `RequestContextMiddleware` for every HTTP request, or adopted from `X-Trace-Id` header |
| Request ID | **Active** | `request_id` (fresh UUID) generated per HTTP request by `RequestContextMiddleware`. Echoed in `X-Request-Id` response header |
| Scope of trace_id | All requests + all tasks | Middleware covers all HTTP requests; Celery tasks receive `trace_id` via task kwargs |
| Cross-service propagation | **Active (Sentry)** | Sentry CeleryIntegration injects `sentry-trace` headers into Celery messages. OTel W3C propagation not yet active (requires `OTLPIntegration`) |
| Frontend → Backend | **Active** | Axios interceptor sends `X-Trace-Id` (per-request UUID) and `X-Session-Id` (per-browser-session, localStorage) on every request |
| Provider spans | **Created but dropped** | `provider_span()` in `providers/tracing.py` creates OTel spans with `ai.provider`, `ai.operation`, `ai.model` attributes. These are silently dropped because Sentry's `instrumenter="sentry"` ignores OTel spans. Fix: enable `OTLPIntegration` in `sentry_config.py` |

**Remaining gap:** OTel provider spans (`provider_span()`) are created but never exported. Sentry handles its own spans via FastAPI/Celery integrations, but the custom AI provider spans (with model/token attributes) are lost. Enabling `OTLPIntegration` in `sentry_config.py` would forward these to Sentry's trace view.

### Frontend Telemetry

| Aspect | Status |
|---|---|
| Error tracking (Sentry) | **Active** | `@sentry/react` + `@sentry/browser` initialized lazily via `lib/sentry.ts`. DSN fetched from backend `GET /api/settings/sentry-dsn` (no auth). 100% sample rate for initial baseline |
| Error boundaries | **Active** | Route-level (`app/error.tsx`) and root-level (`app/global-error.tsx`). Both capture to Sentry before rendering fallback UI |
| Observability helpers | **Active** | `lib/observability.ts` provides: `setUser()`, `trackNavigation()`, `trackAction()`, `trackFunnelStep()`, `trackSlowRequest()`, `captureError()` — all no-op if Sentry not initialized |
| Browser tracing | **Active** | `browserTracingIntegration` with `enableLongAnimationFrame` for performance traces |
| Session tracking | **Active** | `session_id` from localStorage tagged in Sentry events |
| Analytics | **None** (no GA, Mixpanel, PostHog, Amplitude) |
| Web Vitals / RUM | **Partial** | Basic performance via Sentry `browserTracingIntegration`. No dedicated Web Vitals collection (LCP, FID, CLS, INP) |
| Session replay | **Deferred** | Planned for Phase 3 (~40KB gzipped bundle cost). Code comment in `sentry.ts` |
| Unhandled rejection handler | **Active** | Sentry SDK captures unhandled promise rejections by default |

User-facing error feedback is still primarily through Sonner toasts, but errors are now also captured in Sentry for debugging.
...
|---|---|---|---|---|
| `trace_id` | Distributed (cross-service) | `RequestContextMiddleware` (adopts `X-Trace-Id` from frontend or generates new) | Echoed in `X-Trace-Id` response header. Sent to Celery via task kwargs. Stored in pipeline_logs, cost_decisions, usage_records. Injected into all log records via `TraceIdFilter` | **Implemented** — middleware covers all HTTP requests; tasks receive via kwargs |
| `request_id` | Per-HTTP-request | `RequestContextMiddleware` (always fresh UUID) | Echoed in `X-Request-Id` response header. Logged locally | **Implemented** — `middleware/request_context.py` |
| `session_id` | Browser session | Frontend `localStorage` UUID | Sent as `X-Session-Id` header on every Axios request. Extracted by middleware, tagged in Sentry scope | **Implemented** — `lib/api.ts` interceptor + `middleware/request_context.py` |
| `user_id` | Auth scope | JWT decode | All logs, all DB queries | **Implemented** — integer, stored raw (see [Privacy Model](#i-privacy-model--per-user-telemetry-toggle) for future redaction) |
| `job_id` | Job scope | `Job` table PK | Task context, logs, billing | **Implemented** and well-propagated |
| `upload_id` | Upload batch scope | Frontend per-upload | Groups all chunks of one upload operation | **Not implemented** |
| `celery_task_id` | Task scope | Celery auto-generated | Job table, Celery result backend | **Implemented** in `Job.celery_task_id` |
...
### Phase 0: Correlation + Error Capture ~~(do this first, ~1 week)~~ DONE

> **Status: COMPLETE (March 2026).** All Phase 0 tasks implemented.

| # | Task | Status | Implementation |
|---|---|---|---|
| 0.1 | **FastAPI request middleware** | **Done** | `middleware/request_context.py` — pure ASGI middleware generates `trace_id` (adopts `X-Trace-Id` or creates new) + `request_id` per request. Sets billing context. Echoes both as response headers. Tags Sentry scope with `app.trace_id` |
| 0.2 | **Frontend `session_id`** | **Done** | `lib/api.ts` — UUID generated in `localStorage`, sent as `X-Session-Id` header on every Axios request |
| 0.3 | **Frontend `trace_id` capture** | **Done** | `lib/api.ts` — `X-Trace-Id` generated per request, sent to backend. Sentry events tagged with session_id |
| 0.4 | **Install Sentry (backend)** | **Done** | `sentry-sdk[fastapi,celery]` initialized in `main.py` (`_init_sentry()`). DSN from DB (`APIKey` model). `CeleryIntegration(propagate_traces=True, monitor_beat_tasks=True)`. Tags: `app.trace_id`, `app.job_id`, `app.user_id`. Sample rate: 1.0 (baseline). Re-initialized per Celery worker via `worker_ready` signal |
| 0.5 | **Install Sentry (frontend)** | **Done** | `@sentry/react` + `@sentry/browser` in `lib/sentry.ts`. Lazy init: fetches DSN from `GET /api/settings/sentry-dsn`. `browserTracingIntegration` enabled. Session ID tagged |
| 0.6 | **Error boundaries** | **Done** | `app/error.tsx` (route-level) + `app/global-error.tsx` (root-level). Both capture to Sentry |
| 0.7 | **Upgrade health check** | **Done** | `GET /health` checks DB (SELECT 1), Redis (PING), storage (directory exists / container accessible). Returns `healthy` / `degraded` |

**Outcome achieved:** Any Sentry error → `trace_id` → grep backend logs → full request → spawned Celery task chain. Health checks verify all dependencies.

### Phase 1: Distributed Tracing + Worker Correlation + Queue Visibility (2-3 weeks) — PARTIAL

> **Status: PARTIALLY COMPLETE (March 2026).** Core tracing done. Queue metrics and JSON logging remain.

| # | Task | Status | Detail |
|---|---|---|---|
| 1.1 | **Pass `trace_id` to Celery tasks** | **Done** | `dispatch()` helper in `workers/dispatch.py` passes `trace_id` via task kwargs. Tasks call `set_trace_id()` at startup. Sentry `CeleryIntegration(propagate_traces=True)` also injects `sentry-trace` headers |
| 1.2 | **Pass `session_id` + `job_id` to Celery tasks** | **Partial** | `job_id` passed and used in all task context. `session_id` not propagated to workers (stays in API context only) |
| 1.3 | **Install OpenTelemetry Python SDK** | **Done** | `core/otel.py` — `TracerProvider` + `MeterProvider`. Auto-instruments: SQLAlchemy, Redis, HTTPX, Requests, FastAPI. **Note:** With Sentry's default `instrumenter="sentry"`, OTel spans are silently dropped. Sentry owns the span lifecycle |
| 1.4 | **Instrument provider calls as OTel spans** | **Done (spans dropped)** | `providers/tracing.py` — `provider_span()` creates spans with `ai.provider`, `ai.operation`, `ai.model` attributes. Used in all 4 provider files. Spans created but **not exported** (see 1.3 note). Fix: enable `OTLPIntegration` |
| 1.5 | **Add queue depth monitoring** | **Not done** | No Celery beat task for `LLEN` on queues |
| 1.6 | **Add queue wait time metric** | **Not done** | `started_at - created_at` not computed as a metric |
| 1.7 | **Structured JSON logging** | **Not done** | Still plain-text `logging.basicConfig` format. `TraceIdFilter` injects trace_id into log records but output is plain text |
| 1.8 | **SQLAlchemy slow query logging** | **Not done** | No `before_cursor_execute` / `after_cursor_execute` event hooks |

**Remaining work:** 1.5 (queue depth), 1.6 (queue wait time), 1.7 (JSON logging), 1.8 (slow query logging). Also: enable `OTLPIntegration` in `sentry_config.py` to export provider spans to Sentry.
